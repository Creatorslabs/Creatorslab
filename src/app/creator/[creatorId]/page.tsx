...
const unfollow = async () => {
  ...
  if (mainUser?.balance < 5) throw new Error("You dont have enough balance to unfollow!")
                ...
                <div className="creator-profile-img" style={{ width: "100%" }}>
                  ...
                  <div className="flex items-center flex-col md:flex-row justify-between flex-1 flex-wrap gap-4">
                    ...
                    <div className="flex gap-2 justify-center items-center">
                      <button onClick={isFollowing ? () => setUnfollowModal(true) : () => setFollowModal(true)}
  disabled={loading} className="bg-white bg-opacity-20 w-fit h-fit p-2 sm:px-4 rounded-lg flex gap-2 items-center text-sm text-white">
                        <span> {loading ? "Processing..." : isFollowing ? "Unfollow" : "Follow"}</span>
                      </button>
      ...
      <p className="mt-1 text-sm">
        If you follow a creator, you will earn <span className="text-green-400">0.40$CLS</span>.
        But if you decide to unfollow this creator, you will be charged 
        ...
        <CustomModal isOpen={unfollowModal} onClose={() => setUnfollowModal(false)}>
          <div className="flex flex-col gap-3 justify-center items-center dark:text-white">
            <p className="font-bold">Unfollow Creator</p>
            <div className="border border-red-500 text-white bg-red-800/40 dark:bg-red-800/20 p-4 rounded-md text-center max-w-md mx-auto">
            <p className="text-red-400 font-semibold">Please Note</p>
            <p className="mt-1 text-sm">
              If you decide to <span className="text-red-500 font-bold">unfollow</span> this creator, 
              you will be charged <span className="text-red-500 font-bold">5$CLS</span>.
            </p>
            </div>
            <button
            onClick={unfollow}
